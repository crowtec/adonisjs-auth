...
import {
  ...
  CognitoGuardConfig,
  CognitoGuardContract,
  CognitoAuthenticateEventData,
 ...
 */
export class CognitoGuard extends BaseGuard<any> implements CognitoGuardContract<any, any> {
  constructor(
    name: string,
   ...
   * - HTTP context
   */
  private getAuthenticateEventData(user: any): CognitoAuthenticateEventData<any> {
    return {
      name: this.name,
      ctx: this.ctx,
      user,
